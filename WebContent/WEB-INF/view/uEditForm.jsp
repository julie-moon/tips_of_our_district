...
<head>
    ...
    <style>
        ...
        #id, #password, #checkPwd,#nickname,#email,#birthYear{
            margin: 10px auto;
            width: 400px;
        }
        span.warringMsg{
        ...
        .active_school,.active_home,.active_job{
            width: 120px;
            font-size: 25px;
        ...
        .under_one,.under_three,.under_five,.over_five{
            vertical-align: middle;
            width: 180px;
            display: inline-block;
            /*   background: #FEBD39;*/
        ...
        button.exit_btn {
            font:500 25px 'Noto Sans KR',sans-serif;
        }
        .areaBtn{
            margin: 10px;
        }
...
<c:import url="/WEB-INF/view/template/header.jsp"></c:import>
            <h2>정보수정</h2>
            <div id="beeProfile">
                <img class="beeProfile" src="/img/profile_default.png"  width="100px" height="100px" />
                <label>
                    <input id="uploadProfile" title="사진을 등록하세요" type="file" accept="image/*" />
                </label>
            </div>
            <div id="formBox">
                <form method="post" action="main.html">
                    <fieldset>
                        <div class="row">
                            <input id="id" title="아이디를 입력하세요" placeholder="아이디 입력" disabled />
                        </div> <!-- .none_row -->
                        <div class="row">
                            <input id="password" title="비밀번호를 입력하세요" placeholder="비밀번호 입력" />
                            <div class="failMsg">비밀번호는 영문+숫자로 8자 이상 입력해 주세요.</div>
                            <div class="successMsg">비밀번호 조건에 부합합니다.</div>
                        </div><!--//.row_pwd -->
                        <div class="row">
                            <input id="checkPwd" title="비밀번호를 다시 입력하세요" placeholder="비밀번호 확인" />
                            <div class="failMsg">위에 입력하신 비밀번호와 일치하지 않습니다.</div>
                            <div class="successMsg">위에 입력하신 비밀번호와 일치합니다.</div>
                        </div><!--//.row -->
                        <div class="row">
                            <input id="nickname" maxlength="5" title="닉네임을 입력하세요" placeholder="닉네임 입력" />
                            <div class="failMsg">이미 사용중인 닉네임입니다.</div>
                            <div class="successMsg">멋진 닉네임입니다.</div>
                        </div><!--//.row -->
                        <div class="row">
                            <input id="email" title="이메일을 입력하세요" placeholder="이메일 입력"  disabled/>
                        </div><!--//.row -->
                        <div class="row">
                            <input id="birthYear" title="출생년도를 입력하세요" placeholder="출생년도 입력(숫자로만)" disabled />
                        </div><!--//.row -->
                        <div class="row_gender">
                            <label class="gender">성 별</label>
                            <span id="gender">
				        	<input type="radio"  id="female" name="gender"/>
				        	<label for="female" class="female"><i class="fa fa-female"></i> 여 자</label>
				        	<input type="radio" id="male" name="gender"/>
					        <label for="male" class="male"><i class="fa fa-male"></i> 남 자</label>
					        </span>
                        </div><!--//.row -->
                        <div class="row">
                            <span class="warringMsg">지역을 수정하시면 포인트가 차감됩니다.</span>
                            <button type="button" class="btn areaBtn">
                                지역 선택
                            </button>
                        </div><!--//.row -->
                        <div class="row">
                            <div class="active">활동지역</div>
                            <span id="activePlace">
                                <label class="active_school"><input type="radio" name="activePlace" />학교</label>
                                <label class="active_home"><input type="radio" name="activePlace" />집</label>
                                <label class="active_job"><input type="radio" name="activePlace" />직장</label>
                            </span>
                        </div><!--//.row -->
                        <div class="row">
                            <span class="period">
                            <span class="period_select">선택지역 활동기간</span>
                                <label class="under_one" ><input type="radio" name="period"/>1년 미만</label>
                                <label class="under_three"><input type="radio" name="period"/>1 ~ 2년</label><br />
                                <label class="under_five"><input type="radio" name="period"/>3 ~ 5년</label>
                                <label class="over_five"><input type="radio" name="period"/>5년 이상</label>
                            </span>
                        </div>
                        <div id="btnBox">
                            <button type="submit" class="btn joinBtn">
                                수정하기
                            </button>
                        </div> <!-- .btnBox -->
                        <div class="exit">
                            <button type="button" class="exitMember">회원탈퇴</button>
                        </div>
                    </fieldset>
                </form>
            </div> <!--.formBox -->
...
<div class="selectRegion">
    <div class="choiceRegion">
        <label class="choiceRegion">지역선택</label>
    </div><!-- .choiceRegion-->
    <div class="province">
        <div id="popMap">
            <h2>구 선택</h2>
...
C730.596,766.053,735.563,764.919,737.012,767.305z"/>
    ...
    <button type="submit" class="popupBtn">선택 완료</button>
</div>
<!-- 지역선택팝업 -->
<div class="popup">
</div><!--// #popup-->
...
<div id="exitPopup" class="popup">
    ...
    <div class="exitConfirm">
        <span>탈퇴 하시겠습니까?</span>
    </div> <!-- .exitConfirm -->
    <div class="exitCheck">
        <a href="main.gt" class="exit_btn">확인</a>
        <button type="button" class="exit_btn">취소</button>
    </div> <!-- .exitCheck -->
    </div> <!-- .exitPopup -->
</div> <!-- #exitPopup -->
...
<script>
   ...
   $('.areaBtn').click(function (e) {
    ...
    $('.popupBtn').click(function () {
        confirm("포인트가 xxx점 차감됩니다. 차감하시겠습니까?");
        $(".popup").hide();
        $(".selectRegion").hide();
    }); // #regionPopup click() end
